...
def send_mail(SENDERS, RECEIVERS, SUBJECT, BODY, files=[], SERVER="localhost", PORT=587, USERNAME="", PASSWORD="", use_tls=True):

    mail = MIMEMultipart()
    mail['From'] = SENDERS
    mail['To'] = COMMASPACE.join(RECEIVERS)
    ...
    smtp.sendmail(SENDERS, RECEIVERS, mail.as_string())
    smtp.quit()

def createEmailBody(_SUMMARY_ARRAY, _OLD_CVE_ARRAY, CBW_URL = None):
    body = """
    <!doctype html>
    <html>
    <head>
    ...
    for data in _SUMMARY_ARRAY:
        ...
        """.format(
            str(data["GROUP"]["name"]),
            str(data["TOTAL"]),
            str(data["UNMAINTAINED"]),
            str(data["TOTAL"] - data["UNMAINTAINED"]),
            str(data["UNPATCHED"])
        )
    
    # Table : CVEs unpatched for more than 2 months

    body += """
        <h4>CVEs unpatched for more than 2 months</h4>
        <table>
            <tr>
                <th><b>CVE</b></th>
                <th><b>ASSET</b></th>
    ...
    for data in _OLD_CVE_ARRAY:
        body += """
            <tr>
                <td><a href="{}/cve_announcements/{}">{}</a></td>
        ...
        """.format(
            ...
            str(data["CVE_CODE"]),
            CBW_URL,
            str(data["AFFECTED_ASSET_ID"]),
            str(data["AFFECTED_ASSET"]),
            str(data["CVSS"]),
            str(data["DETECTED_AT"]),
        )
